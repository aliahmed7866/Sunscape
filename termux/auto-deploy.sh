...
SERVICE="${SUNSCAPE_SERVICE:-sunscape}"
PORT="${SUNSCAPE_PORT:-8081}"
INTERVAL="${SUNSCAPE_DEPLOY_INTERVAL:-60}"
STATE_DIR="${SUNSCAPE_DEPLOY_STATE_DIR:-$HOME/.local/state/sunscape}"
LOG_PREFIX="[Sunscape deploy]"
...
health_ok() {
  curl -fsS --max-time 8 "http://127.0.0.1:$PORT/health" >/dev/null 2>&1
}
