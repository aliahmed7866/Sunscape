(() => {
  ...
  button.type = "button";
  button.textContent = "Preparing Sunscape…";
  button.setAttribute("aria-label", "Install Sunscape on this phone");
  Object.assign(button.style, {
    position: "fixed", right: "14px", bottom: "calc(18px + env(safe-area-inset-bottom))",
    zIndex: "9999", display: "block", padding: "11px 15px", border: "1px solid #f1aa60",
    borderRadius: "999px", background: "#96612d", color: "#fff", fontWeight: "800",
  ...
  window.addEventListener("beforeinstallprompt", event => {
    ...
    promptEvent = event;
    button.textContent = "Install Sunscape";
  });
  ...
  async function prepare() {
    if (!("serviceWorker" in navigator) || !window.isSecureContext) {
      button.textContent = "Check Sunscape install";
      return;
    }
    try {
    ...
    setTimeout(() => {
      if (!promptEvent) button.textContent = "Check Sunscape install";
    }, 1500);
  }
